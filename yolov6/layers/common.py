...
class psu(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=None, groups=1, bias=False):
        super().__init__()
        c_ = out_channels // 2
        self.cv1 = ConvBNSiLU(in_channels, out_channels, kernel_size, 2, padding, groups, bias)
        self.cv2 = ConvBNSiLU(in_channels, c_, kernel_size, stride, padding, groups, bias)
        self.cv3 = RepVGGBlock(in_channels, c_, kernel_size, stride,1 , groups, bias)
        self.cv4 = ConvBNSiLU(c_* 2,out_channels,1, stride, padding, groups, bias)

    def forward(self, x):
        y=self.cv1 (x)
        return self.cv4(torch.cat((self.v3(self.v2(y)), self.cv2(y)), 1))
...
class RepVGGSiBlock(nn.Module):
    '''RepVGGBlock is a basic rep-style block, including training and deploy status
    This code is based on https://github.com/DingXiaoH/RepVGG/blob/main/repvgg.py
    '''
    def __init__(self, in_channels, out_channels, kernel_size=3,
                 stride=1, padding=1, dilation=1, groups=1, padding_mode='zeros', deploy=False, use_se=False):
        super(RepVGGSiBlock, self).__init__()
