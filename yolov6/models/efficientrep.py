...
from yolov6.layers.common import BottleRep, RepVGGBlock, RepBlock, BepC3, SimSPPF, SPPF, SimCSPSPPF, CSPSPPF, ConvBNSiLU, \
                                MBLABlock, ConvBNHS, Lite_EffiBlockS2, Lite_EffiBlockS1,psu,RepVGGSiBlock


class tpsu(nn.Module):
    ...
    def __init__(
        ...
        self.in1 = nn.Sequential(
            block(
                ...
                out_channels=channels_list[1], kernel_size=3, stride=2
            ),block(
                in_channels=channels_list[1],
                out_channels=channels_list[0], kernel_size=1, stride=1
            )
        )
        self.re1=nn.Sequential(
            RepVGGSiBlock(
                in_channels=channels_list[0],
                out_channels=channels_list[0], kernel_size=3, stride=1
            )
        )
        self.out1 = nn.Sequential(
            block(
                in_channels=channels_list[1],
                out_channels=channels_list[1], kernel_size=1, stride=1
            )
        )
        self.in2 = nn.Sequential(
            block(
                in_channels=channels_list[1],
                out_channels=channels_list[2], kernel_size=3, stride=2
            ),block(
        ...
        self.re2=nn.Sequential(
            RepVGGSiBlock(
                in_channels=channels_list[1],
                out_channels=channels_list[1], kernel_size=3, stride=1
            )
        )
        self.out2 = nn.Sequential(
            block(
                in_channels=channels_list[2],
                out_channels=channels_list[2], kernel_size=1, stride=1
            )
        )      
        self.in3 = nn.Sequential(
            block(
                in_channels=channels_list[2],
                out_channels=channels_list[3], kernel_size=3, stride=2
            ),block(
        ...
        self.re3=nn.Sequential(
            RepVGGSiBlock(
                in_channels=channels_list[2],
                out_channels=channels_list[2], kernel_size=3, stride=1
            )
        )
        self.out3 = nn.Sequential(
            block(
                in_channels=channels_list[3],
                out_channels=channels_list[3], kernel_size=1, stride=1
            )
        )      
        self.in4 = nn.Sequential(
            block(
                in_channels=channels_list[3],
                out_channels=channels_list[4], kernel_size=3, stride=2
            ),block(
                in_channels=channels_list[4],
                out_channels=channels_list[3], kernel_size=1, stride=1
            )
        )
        self.re4=nn.Sequential(
            RepVGGSiBlock(
                in_channels=channels_list[3],
                out_channels=channels_list[3], kernel_size=3, stride=1
            )
        )
        self.out4 = nn.Sequential(
            block(
                in_channels=channels_list[4],
                out_channels=channels_list[4], kernel_size=1, stride=1
            )
        )      
